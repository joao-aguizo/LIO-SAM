...
if [ "$color_prompt" = yes ]; then
    PS1='[Singularity] ${debian_chroot:+($debian_chroot)}\[\033[01;32m\]\u@\h\[\033[00m\]:\[\033[01;34m\]\w\[\033[00m\]\$ '
else
    PS1='[Singularity] ${debian_chroot:+($debian_chroot)}\u@\h:\w\$ '
fi
...
# source the distro by default
if [ -e /opt/ros/$ROS_DISTRO/setup.bash ]; then
  source /opt/ros/$ROS_DISTRO/setup.bash
fi

# source the user_workspace, if it exists
[ -e ~/singularity_ws/devel/setup.bash ] && source ~/singularity_ws/devel/setup.bash
