...
SINGULARITY_PATH=`dirname "$0"`
SINGULARITY_PATH=`( cd "$SINGULARITY_PATH" && pwd )`
PACKAGE_PATH=`( cd "$SINGULARITY_PATH/../." && pwd )`
PACKAGE_NAME=`( basename "$PACKAGE_PATH" )`

# alternatively, set it directly
# SINGULARITY_PATH=$HOME/git/singularity

# define paths to the subfolders
IMAGES_PATH="$SINGULARITY_PATH/images"
...
CONTAINER_NAME="image.sif"
OVERLAY_NAME="image.img"
...
USE_NVIDIA=false # true: will tell Singularity to use nvidia graphics (not always works).
...
MOUNTS=(
  ...
  "type=bind" "$PACKAGE_PATH" "/home/$USER/singularity_ws/src/$PACKAGE_NAME"

  # mount the MRS shell additions into the container, DO NOT MODIFY
  "type=bind" "$MOUNT_PATH" "/opt/image/host"
)
...
# check whether we are already in tmux and action is run (it is not allowed)
if [ -n "$TMUX" ] && [ ! "$ACTION" = "exec" ]; then
